...
## Thành viên Nhóm
- **Nhóm trưởng:** [Tên Nhóm trưởng (BMT-wq)]
- **Thành viên 1:** [Tên Thành viên A]
- **Thành viên 2:** [Tên Thành viên B]
- **Thành viên 3:** [Tên Thành viên C]
- **Thành viên 4:** [Tên của bạn]
...
2.  **SortedList<DateTime, List<Thuoc>> (Tìm theo HSD O(log N)):**
    - **File:** `KhoManager.cs`
    - **Giải thích:** Sử dụng SortedList (Cây nhị phân) để tự động sắp xếp thuốc theo Hạn sử dụng. Điều này cho phép tìm kiếm thuốc sắp hết hạn (ví dụ: trong 30 ngày tới) cực kỳ nhanh với tốc độ O(log N).

3.  **Tối ưu Cập nhật (O(1)):**
    - **File:** `Thuoc.cs`
    - **Giải thích:** Dùng `class Thuoc` (kiểu tham chiếu). Khi cập nhật số lượng, hệ thống chỉ cần tìm O(1) trong Dictionary và thay đổi giá trị `SoLuongTon` tại 1 địa chỉ duy nhất, không cần cập nhật lại 2 cấu trúc dữ liệu.
